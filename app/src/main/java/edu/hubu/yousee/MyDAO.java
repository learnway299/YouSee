...
public class MyDAO {
    ...
    public Cursor allQuery(){    //查询所有记录
        ...
        return myDb.rawQuery("select * from myitem",null);
    }
    public  int getRecordsNumber(){  //返回数据表记录数
        myDb = dbHelper.getReadableDatabase();
        Cursor cursor= myDb.rawQuery("select * from myitem",null);
        return cursor.getCount();
    }

    public void insertInfo(String name,int price){  //插入记录
    ...
    public void updateInfo(String name,int price,String selId){  //修改记录
        //方法中的第三参数用于修改选定的记录
        ContentValues values = new ContentValues();
        values.put("name", name);
        values.put("price", price);
        ...
        //myDb.execSQL("update friends set name = ? ,price = ? where _id = ?",new Object[]{name,age,selId});
